...
const ProjectCard = ({ proyecto }: { proyecto: ProyectoType }) => {
  ...
  return (
    <li className="list-row border border-base-300 hover:border-primary/30 hover:shadow-2xl hover:scale-105 transition-all duration-300 cursor-pointer">
      {/* Avatar/Icono del proyecto */}
      <div className="avatar avatar-placeholder">
        <div className="w-12 rounded-full bg-primary text-primary-content">
          <span className="text-lg font-bold">{proyecto.codigo.slice(0, 2)}</span>
        </div>
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className={`badge badge-sm ${getEstadoColor(proyecto.estado)}`}>
            {proyecto.estado}
          </span>
        </div>
      ...
      <div className="text-right min-w-0">
        ...
        <div className="text-xs opacity-60 mt-0.5">{progressPercentage}%</div>
      </div>
    </li>
  );
};
